...
const INVOICE_FROM = (process.env.GREEN_GRIN_INVOICE_FROM || "Green Grin Lawns <ken@greengrinlawns.com>").replace("Green Grin Lawn & Landscape", "Green Grin Lawns");
...
function invoiceEmailPayload(invoice) {
  ...
  const isProjectInvoice = Boolean(invoice.source_estimate_id || invoice.source_estimate_number);
  const itemText = items.map((item) => isProjectInvoice
    ? `${item.description}: $${item.amount.toFixed(2)}`
    : `${item.description}: ${item.quantity} ${item.unit} x $${item.rate.toFixed(2)} = $${item.amount.toFixed(2)}`);
  const itemRows = items.map((item) => isProjectInvoice
    ? `<tr><td style="padding:9px;border-bottom:1px solid #dce7d9">${escapeHtml(item.description)}</td><td style="padding:9px;border-bottom:1px solid #dce7d9;text-align:right;font-weight:bold">$${item.amount.toFixed(2)}</td></tr>`
    : `<tr><td style="padding:9px;border-bottom:1px solid #dce7d9">${escapeHtml(item.description)}</td><td style="padding:9px;border-bottom:1px solid #dce7d9;text-align:right">${escapeHtml(`${item.quantity} ${item.unit}`)}</td><td style="padding:9px;border-bottom:1px solid #dce7d9;text-align:right">$${item.rate.toFixed(2)}</td><td style="padding:9px;border-bottom:1px solid #dce7d9;text-align:right;font-weight:bold">$${item.amount.toFixed(2)}</td></tr>`).join("");
